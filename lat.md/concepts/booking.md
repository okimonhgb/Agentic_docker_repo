...
- **Nearest-alternative state propagation**: `requested_slot_start`, `nearest_before_start`, and `nearest_after_start` are declared in `GraphState` and included in `_BOOKING_STATE_KEYS` / `_booking_preserve_keys` so they survive the subgraph boundary from `availability_agent` to `booking_agent`. Without this, LangGraph silently dropped these fields and the booking agent couldn't show the "requested time unavailable, nearest is X" note.
